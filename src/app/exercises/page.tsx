...
export default function ExercisesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const libRes = await supabase
        ...
        .select('id, name, description, instructions, default_sets, default_reps, default_duration_s, category, warning, order_index, is_active')
        .eq('is_active', true)
        .order('order_index')
      ...
      setExercises((libRes.data ?? []) as Exercise[])
      setTodayLogs((logsRes.data ?? []) as ExerciseLog[])
      setLoading(false)
    }
    load()
  }, [today])
  ...
  async function markDone(exercise: Exercise, skipped = false) {
    ...
    const insertData = {
      ...
      log_date: today,
      sets_completed: skipped ? null : (exercise.default_sets ?? null),
      reps_completed: skipped ? null : (exercise.default_reps ?? null),
      duration_s: skipped ? null : (exercise.default_duration_s ?? null),
      skipped,
      skip_reason: skipped ? 'user_skipped' : null,
    }

    // Use any to bypass Supabase generated type inference issue
    await (supabase.from('exercise_logs') as any).insert(insertData)
    ...
    <div className="min-h-screen bg-surface pb-36">
      ...
      <div className="px-6 py-4">
        <div className="flex justify-between items-center mb-2">
          <span className="font-label text-xs uppercase tracking-widest text-on-surface-variant">Today&apos;s progress</span>
          <span className="font-label text-xs text-primary">{doneCount} / {exercises.length} done</span>
        </div>
                  ...
                  >
                    ...
                    >
                      <div className={clsx(
                        ...
                        isDone ? 'bg-primary border-primary' : 'border-outline/30 bg-transparent'
                      )}>
                        {isDone && (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={clsx('font-headline font-semibold text-sm', isDone ? 'text-primary' : 'text-on-surface')}>
                          {exercise.name}
                        </p>
                        <p className="font-label text-xs text-on-surface-variant mt-0.5 truncate">{exercise.description}</p>
                      </div>
                      <div className="flex items-center gap-3 flex-shrink-0">
                        {exercise.default_sets && exercise.default_reps && (
                          <span className="font-label text-xs text-on-surface-variant">{exercise.default_sets}×{exercise.default_reps}</span>
                        )}
                        {exercise.default_duration_s && (
                          <span className="font-label text-xs text-on-surface-variant">{exercise.default_duration_s}s</span>
                        )}
                    ...
                    {isExpanded && (
                      <div className="px-5 pb-5 space-y-4 border-t border-outline/10 pt-4">
                        <div className="flex items-start gap-2 bg-primary/5 rounded-xl p-3">
                          ...
                          <p className="font-label text-xs text-primary">{exercise.warning}</p>
                        </div>
                        <p className="font-body text-sm text-on-surface-variant leading-relaxed">{exercise.instructions}</p>
                        <div className="flex gap-3">
                          <button
                            ...
                            className={clsx(
                              ...
                              isDone ? 'bg-primary/20 text-primary' : 'bg-gradient-to-r from-primary to-primary-container text-on-primary'
                            )}
                          >
