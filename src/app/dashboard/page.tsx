...
type PainLogRow = { log_date: string; intensity: number }
...
export default async function DashboardPage() {
  ...
  const recentLogs = (logsRes.data ?? []) as PainLogRow[]
  ...
  return (
    <div className="min-h-screen bg-surface pb-32">
      ...
      <main className="px-6 space-y-4">
        ...
        {!loggedToday ? (
          <Link href="/log">
            <div className="bg-primary-container/20 border border-primary/20 rounded-2xl p-6 flex items-center justify-between group hover:bg-primary-container/30 transition-colors">
              <div>
                <p className="font-label text-xs uppercase tracking-widest text-primary mb-1">Not logged today</p>
                <p className="font-headline font-semibold text-on-surface">How is your pain today?</p>
              </div>
              <span className="material-symbols-outlined text-primary text-3xl group-hover:translate-x-1 transition-transform">arrow_forward</span>
            </div>
          </Link>
        ) : (
          <div className="bg-surface-container rounded-2xl p-6">
            <p className="font-label text-xs uppercase tracking-widest text-primary mb-1">Logged today</p>
            <div className="flex items-end gap-2 mt-1">
              <span className="text-4xl font-extrabold font-headline text-primary">{lastLog?.intensity}</span>
        ...
        <div className="bg-surface-container rounded-2xl p-6">
          <p className="font-label text-xs uppercase tracking-widest text-on-surface-variant mb-4">Last 7 days</p>
          {recentLogs.length > 0 ? (
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div
                      ...
                      style={{
                        height: `${height}%`,
                        backgroundColor: intensity ? `rgba(255, 200, 128, ${0.3 + (intensity / 10) * 0.7})` : '#191e35',
                        minHeight: '4px'
                      }}
                    />
            ...
            <p className="text-on-surface-variant text-sm font-label">Start logging to see your pain trends here.</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Link href="/exercises">
            <div className="bg-surface-container rounded-2xl p-5 hover:bg-surface-container-high transition-colors">
              <span className="material-symbols-outlined text-primary text-2xl mb-3 block" style={{ fontVariationSettings: "'FILL' 1" }}>fitness_center</span>
          ...
          <Link href="/trends">
            <div className="bg-surface-container rounded-2xl p-5 hover:bg-surface-container-high transition-colors">
              <span className="material-symbols-outlined text-primary text-2xl mb-3 block" style={{ fontVariationSettings: "'FILL' 1" }}>query_stats</span>
